...
class AbstractIdempotencyServiceTest {
    ...
    void canRetry_NullRetryCount_ReturnsTrue() {
        IdempotencyKey entity = IdempotencyKey.builder().key("test-key").retryCount(null).build();
    ...
    void canRetry_BelowMaxRetries_ReturnsTrue() {
        IdempotencyKey entity = IdempotencyKey.builder().key("test-key").retryCount(3).build();
    ...
    void canRetry_AtMaxRetries_ReturnsFalse() {
        IdempotencyKey entity = IdempotencyKey.builder().key("test-key").retryCount(5).build();
    ...
    void retry_ExistingRetryCount_Increments() {
        IdempotencyKey entity = IdempotencyKey.builder().key("test-key").retryCount(2).build();
    ...
    void fail_Entity_SetsFailedStatus() {
        IdempotencyKey entity = IdempotencyKey.builder().key("test-key").status(IdempotencyStatus.IN_PROGRESS).build();
    ...
    void isFailed_FailedEntity_ReturnsTrue() {
        IdempotencyKey entity = IdempotencyKey.builder().key("test-key").status(IdempotencyStatus.FAILED).build();
    ...
    void isFailed_NonFailedEntity_ReturnsFalse() {
        IdempotencyKey entity = IdempotencyKey.builder().key("test-key").status(IdempotencyStatus.COMPLETED).build();
