...
/**
 * PayOS webhook handler service
 * Processes webhook notifications from PayOS
 */
@Service
...
public class PayosWebhookService {
    ...
    public void handle(PayosWebhookRequest webhookRequest) {
        ...
        if (webhookRequest.getSignature() != null) {
            boolean isValid = signatureUtil.validateWebhookSignature(webhookRequest.getData(),
                    webhookRequest.getSignature());

            if (!isValid) {
                log.error("PayOS webhook signature validation failed for order: {}", orderCode);
